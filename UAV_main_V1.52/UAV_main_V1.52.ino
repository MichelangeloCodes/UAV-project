...
int MaxonLinks_pwm = 100;
int MaxonRechts_pwm = 30;
int Farnell_pwm = 0;
...
void setup() {
  ...
  digitalWrite(RelaisSysteem, HIGH);  //arduino set system on

  digitalWrite(SHT_LOX1, LOW);
  digitalWrite(SHT_LOX2, LOW);
  digitalWrite(SHT_LOX3, LOW);
  Serial.println("reset mode...(pins are low)");
  
  digitalWrite(ledBlauw, HIGH);       //setup - wait until led LOW
...
void loop() {
 if(TimeToStabilize){
   //TOFsensor();    //TOF sensor meassure distance
   maxonMotorLinks(1, MaxonLinks_pwm);
   maxonMotorRechts(2, MaxonRechts_pwm);
...
ISR(TIMER1_COMPA_vect) {
  ...
  if (count >= 100){
    MeassureBatteryVoltage();   // read voltage level after 30 sec
    count = 0;
  }
  count++;
}
